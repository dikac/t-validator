...
export default class Filter<Base = unknown, Type extends Base = Base, Extent extends Instance<Base> = Instance<Base>, ValidatorT extends Simple = Simple> implements Simple<Base, Type, Extent>, ValidatorContainer<ValidatorT> {
    ...
    filter: <Argument extends Base>(result: Infer<ValidatorT, Base>, argument: Base) => Validatable<Base, Argument, Type, Extent>;
    constructor(validator: ValidatorT, filter: <Argument extends Base>(result: Infer<ValidatorT, Base>, argument: Base) => Validatable<Base, Argument, Type, Extent>);
